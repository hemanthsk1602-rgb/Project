      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/[0.08]">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 bg-[#0C111C] border border-white/[0.08] p-1 rounded-xl text-xs font-mono relative">
              <button
                onClick={() => setViewMode('tree')}
                className={`relative px-3 py-1.5 rounded-lg transition-colors z-10 ${
                  viewMode === 'tree' ? 'text-white font-semibold' : 'text-zinc-400 hover:text-white'
                }`}
              >
                {viewMode === 'tree' && (
                  <motion.div
              ...
              <button
                ...
                className={`relative px-3 py-1.5 rounded-lg transition-colors z-10 ${
                  viewMode === 'grid' ? 'text-white font-semibold' : 'text-zinc-400 hover:text-white'
                }`}
              >
                {viewMode === 'grid' && (
                  <motion.div
            ...
            <AnimatePresence mode="wait">
              {viewMode === 'tree' ? (
                <motion.div
                  ...
                  className="p-6 sm:p-8 rounded-2xl bg-[#080C14] border border-white/[0.08] shadow-2xl flex flex-col items-center relative overflow-hidden"
                >
                  {/* Subtle Background Glow */}
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-96 h-48 bg-brand-500/5 rounded-full blur-3xl pointer-events-none" />
                  ...
                  <motion.div
                    ...
                    className="px-4 py-1.5 rounded-full bg-white/[0.04] border border-white/[0.12] text-[11px] font-mono font-bold text-zinc-300 tracking-wider shadow-sm"
                  >
                    START • FOUNDATIONS
                  </motion.div>

                  {/* Connecting Line */}
                  <div className="w-px h-6 bg-gradient-to-b from-emerald-500/60 to-emerald-500/30" />
                  ...
                  <motion.div
                    ...
                    initial="hidden"
                    animate="visible"
                    className="w-full max-w-xl space-y-4 flex flex-col items-center"
                  >
                            ...
                            <div className="grid grid-cols-2 gap-4 w-full">
                              ...
                              <motion.button
                                ...
                                className={`p-3.5 rounded-xl border text-left transition-all ${
                                  isLeftSelected
                                    ? 'bg-[#101726] border-brand-500 ring-1 ring-brand-500/50 shadow-[0_0_20px_rgba(139,92,246,0.18)]'
                        ...
                        >
                          <motion.button
                            ...
                            className={`w-full sm:w-96 p-4 rounded-xl border text-left transition-all relative ${
                              isSelected
                                ? 'bg-[#101726] border-brand-500 ring-1 ring-brand-500/50 shadow-[0_0_24px_rgba(139,92,246,0.2)]'
                                : isCompleted
                                ? 'bg-[#090E18] border-emerald-500/30 hover:border-emerald-500/50'
                                : isInProgress
                                ? 'bg-[#0A0F1D] border-brand-500/30 hover:border-brand-500/50'
                  ...
                  {/* Connector to Master */}
                  <div className="w-px h-6 bg-gradient-to-b from-white/10 to-brand-500/40 mt-1" />

                  {/* MASTER Node */}
                  <motion.div
                    whileHover={{ scale: 1.04 }}
                    className="px-5 py-2 rounded-full bg-brand-500/10 border border-brand-500/40 text-xs font-mono font-bold text-brand-300 tracking-wider shadow-glow-brand flex items-center gap-2"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-brand-400" />
                    <span>DSA MASTER REACHED</span>
                  </motion.div>
                </motion.div>
              ) : (
                /* Module Grid View */
                      ...
                      selectedTopic.problemSlugs.map((slug) => (
                        ...
                        >
                          ...
                          >
                            ...
                            <ArrowRight className="w-3 h-3 text-zinc-500 group-hover:text-white transition-transform group-hover:translate-x-0.5" />
                          </Link>
                        </motion.div>
                      ))
                    ) : (
